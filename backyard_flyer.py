...
class BackyardFlyer(Drone):
    ...
    def local_position_callback(self):
        if self.flight_state == States.TAKEOFF:
            if -1.0 * self.local_position[2] > 0.95 * self.target_position[2]:
                self.all_pathpoints = self.calc_path()
                self.path_pt_transition()
        elif self.flight_state == States.WAYPOINT:
            if np.linalg.norm(self.target_position[0:2] - self.local_position[0:2]) < 1.0:
                if len(self.all_pathpoints) > 0:
                    self.path_pt_transition()
                else:
    ...
    def calc_path(self):
        print("Setting Home")
        local_waypoints = [[12.0, 0.0, 3.0], [12.0, 12.0, 3.0], [0.0, 12.0, 3.0], [0.0, 0.0, 3.0]]
    ...
    def path_pt_transition(self):
        print("path coordinates transition")
        self.target_position = self.all_pathpoints.pop(0)
